  ...
  return (
    <div style={{margin:"10px 0"}}>
      <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:"5px",marginBottom:"8px"}}>
        {[{n:byCat.Fund,l:"Funding",c:"#7eb8ff"},{n:byCat.Pilot,l:"Pilot Sites",c:"#7eff9e"},{n:byCat.Accel,l:"Accelerators",c:"#ffb87e"}].map(function(d,i){
          return <div key={i} style={{background:"#16213e",borderRadius:"8px",padding:"8px",textAlign:"center",border:"1px solid #2a2a4a"}}><div style={{fontSize:"1.3rem",fontWeight:800,color:d.c,...hs}}>{d.n}</div><div style={{fontSize:"0.5rem",color:d.c,...hs,fontWeight:600}}>{d.l}</div></div>;
        })}
      </div>
      <div style={{background:"#16213e",border:"1px solid #2a2a4a",borderRadius:"8px",padding:"8px",marginBottom:"6px"}}>
        <div style={{fontSize:"0.58rem",fontWeight:700,color:"#e94560",marginBottom:"4px",...hs}}>Coverage by Stage</div>
        {SN.map(function(s,i){var ct=byStage[SK[i]];var pct=Math.round(ct/E.length*100);
          return <div key={i} style={{display:"flex",alignItems:"center",gap:"4px",marginBottom:"2px"}}>
            <span style={{fontSize:"0.54rem",width:"55px",textAlign:"right",color:"#aab",...hs}}>{s}</span>
            <div style={{flex:1,height:"7px",background:"#0f0f23",borderRadius:"3px",overflow:"hidden"}}><div style={{height:"100%",width:pct+"%",background:"linear-gradient(90deg,#533483,#e94560)",borderRadius:"3px"}}/></div>
            <span style={{fontSize:"0.52rem",color:"#eee",width:"18px",...hs,fontWeight:600}}>{ct}</span>
          </div>;
        })}
      </div>
      <div style={{background:"#16213e",border:"1px solid #2a2a4a",borderRadius:"8px",padding:"8px"}}>
        <div style={{fontSize:"0.58rem",fontWeight:700,color:"#e94560",marginBottom:"4px",...hs}}>Coverage by Region</div>
        {Object.entries(byRgn).map(function(pair){var k=pair[0],v=pair[1];var pct=Math.round(v/E.length*100);
          return <div key={k} style={{display:"flex",alignItems:"center",gap:"4px",marginBottom:"2px"}}>
            <span style={{fontSize:"0.54rem",width:"55px",textAlign:"right",color:"#aab",...hs}}>{k}</span>
            <div style={{flex:1,height:"7px",background:"#0f0f23",borderRadius:"3px",overflow:"hidden"}}><div style={{height:"100%",width:pct+"%",background:k==="AB"?"#e94560":k==="National"?"#c5a55a":"#533483",borderRadius:"3px"}}/></div>
...
export default function Home() {
  ...
  var th = eco
    ? {bg:"#1a1a2e",hdr:"#0f0f23",acc:"#e94560",ac2:"#533483",card:"#16213e",tx:"#eee",tx2:"#aab",brd:"#2a2a4a"}
  ...
  function fmt(t) {
    return t.split("\n").map(function(ln, i) {
      ...
      if (ln.indexOf("### ") === 0) return <h3 key={i} style={{fontSize:"0.86rem",fontWeight:700,color:eco?"#e94560":"#1a3a0a",margin:"8px 0 3px"}}>{ln.slice(4)}</h3>;
      if (ln.indexOf("## ") === 0) return <h2 key={i} style={{fontSize:"0.92rem",fontWeight:700,color:eco?"#e94560":"#1a3a0a",margin:"12px 0 4px",borderTop:"1px solid "+th.brd,paddingTop:"10px"}}>{ln.slice(3)}</h2>;
    ...
    <div style={{minHeight:"100vh",background:th.bg,fontFamily:"'Source Serif 4',Georgia,serif",display:"flex",flexDirection:"column",transition:"background .3s"}}>
      ...
      <header style={{background:th.hdr,padding:"9px 14px",display:"flex",alignItems:"center",justifyContent:"space-between",borderBottom:"3px solid "+th.acc,flexWrap:"wrap",gap:"5px"}}>
        <div>
          ...
          <p style={{color:th.acc,fontSize:"0.58rem",margin:"1px 0 0",...hs,fontWeight:500}}>{"99 programs \u00B7 " + (eco ? "Ecosystem Intelligence" : "Entrepreneur Mode") + " \u00B7 Alberta + National"}</p>
        </div>
        ...
        {sx && (
          <div>
            <div style={{textAlign:"center",marginBottom:"16px",padding:"8px 0"}}>
              <h2 style={{fontSize:"1.1rem",fontWeight:700,color:eco?"#e94560":"#1a3a0a",margin:"0 0 4px"}}>{eco ? "Where are the gaps?" : "What do you need next?"}</h2>
      ...
      <div style={{borderTop:"1px solid "+th.brd,background:th.card,padding:"8px 14px",maxWidth:"820px",width:"100%",margin:"0 auto",boxSizing:"border-box"}}>
        ...
        <p style={{fontSize:"0.54rem",color:th.tx2,...hs,margin:"3px 0 0",textAlign:"center"}}>Canadian Ag Innovation Navigator V0 - 99 entries - Bioenterprise 2024 + provincial + federal</p>
      </div>
    </div>
  );
}
